<h1 align="center" style="font-weight: bold">
    DCIT50 Lab 4
</h1>

<p align="center">
    <a href="https://github.com/JeremyVienArriola/DCIT50-Lab-6/issues">
        <img src="https://img.shields.io/github/issues/JeremyVienArriola/DCIT50-Lab-6.svg?style=for-the-badge&logo=data:image/png;base64,">
    </a>
    <a href="https://github.com/JeremyVienArriola/DCIT50-Lab-6/network/members">
        <img src="https://img.shields.io/github/forks/JeremyVienArriola/DCIT50-Lab-6.svg?style=for-the-badge&logo=data:image/png;base64,">
    </a>
    <a href="https://github.com/JeremyVienArriola/DCIT50-Lab-6/stargazers">
        <img src="https://img.shields.io/github/stars/JeremyVienArriola/DCIT50-Lab-6.svg?style=for-the-badge&logo=data:image/png;base64,">
    </a>
    <a href="https://github.com/JeremyVienArriola/DCIT50-Lab-6/graphs/contributors">
        <img src="https://img.shields.io/github/contributors/JeremyVienArriola/DCIT50-Lab-6.svg?style=for-the-badge&logo=data:image/png;base64,">
    </a>
    
</p>

<p align="center">
    <a href="LICENSE.md">
        <img src="https://img.shields.io/badge/LICENSE-A31F34?style=for-the-badge&logoWidth=25&logo=data:image/png;base64,">
    </a>
</p>

Hello! This project is about creating and designing a webpage using physical tags, and applying styling in example webpages.

## Getting Started

To get started, follow [this instruction](getting-started.md).

## Linting

This codebase is linted by demons that does not understand how to semantically format that appeases the mortal eyes. Thus, here we are with unsatisfactorally formatted code.

<!-- License section. Leave unchanged except when changing the style altogether. -->
## License

### <a target="_blank" href="https://choosealicense.com/licenses/mit/">MIT License</a>

Copyright 2025 Jeremy Vien G. Arriola [jeremyviengarriola@gmail.com]

Check the [LICENSE](LICENSE.md) for more details.

## Attribution

### MIT Logo

<a target="_blank" href="https://commons.wikimedia.org/wiki/File:MIT_logo.svg">Massachusetts Institute of Technology</a> (vectorized by <a target="_blank" href="https://en.wikipedia.org/wiki/User:Mysid">Mysid</a>, modified by [whinee](https://github.com/whinee)), Public domain, via Wikimedia Commons

### Icons

<a target="_blank" href="https://icons8.com/icon/102502/exclamation-mark">Exclamation Mark</a>, <a target="_blank" href="https://icons8.com/icon/33294/code-fork">Code Fork</a>, <a target="_blank" href="https://icons8.com/icon/85185/star">Star</a>, <a target="_blank" href="https://icons8.com/icon/34095/group">Group</a>, and <a target="_blank" href="https://icons8.com/icon/87276/code">Code</a> icon by <a target="_blank" href="https://icons8.com">Icons8</a>
